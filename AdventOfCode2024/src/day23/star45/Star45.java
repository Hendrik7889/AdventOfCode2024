...
public class Star45 {
    public static void main(String[] args) {
        ...
        List<List<Vertex>> combinations = findNumberOfFullSubGraphs(graph,sizeOfFullSubGraph);
        int count = 0;
        for(List<Vertex> combination : combinations) {
            for(Vertex vertex : combination) {
                if(vertex.label.startsWith("t")) {
                    count++;
                    break;
                }
            }
        }
        System.out.println("Result:" + count/sizeOfFullSubGraph);
    }
